...
#if defined(NDEBUG)
# error "BARIUM cannot be compiled without assertions."
#endif
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // BARIUM: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "BARIUM Signed Message:\n";
...
{
    // BARIUM: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 150000 * COIN;

    // Subsidy is cut in half every 1470000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 1470000); // BARIUM: 1470k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1.5 * 24 * 60 * 60; // BARIUM: 1.5 days
static const int64 nTargetSpacing = 1.5 * 60; // BARIUM: 1.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // BARIUM: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // BARIUM: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xa76573b99cd3de3c777b10f10435a4b448b544b3ffea30e3bfca3fc5c4f89dba");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "24/Mar/2018 World day to combat tuberculosis";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("041084710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1521883800;
        ...
        assert(block.hashMerkleRoot == uint256("0xd91fec73f083824dafe9fffca083fb5da33bbaba59e1280a438707994da02722"));
...
unsigned char pchMessageStart[4] = { 0xc4, 0xd3, 0xd2, 0xbd }; // BARIUM: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BARIUMMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BARIUMMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BARIUMMiner(CWallet *pwallet)
{
    printf("BARIUMMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("BARIUM-miner");
    ...
    try { loop {
        ...
        printf("Running BARIUMMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BARIUMMiner, pwallet));
}
